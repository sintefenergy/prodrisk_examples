...
def get_vol_head_dict(data_dir, model_name):
    model_file = h5py.File(data_dir + "model.h5", 'r')
    ...
    for key in res.keys():
        try:
            ...
            continue
        if "Kote" in res[key]:
            mod_name = get_mod_name_from_mod_nr(data_dir, model_name, int(res[key]))
            vol_head_dict[mod_name] = [np.array(res[key]["Vol"]), np.array(res[key]["Kote"])]

    return vol_head_dict


def get_mod_name_from_mod_nr(data_dir, model_name, mod_nr):
    model_file = h5py.File(data_dir + "model.h5", 'r')
    system_path = 'hydro_data/' + model_name.upper()
    ...
    for i in range(module_data.size):
        if module_data[i]['res_id'] == mod_nr:
            mod_name = module_data[i]['res_name'].decode("iso-8859-1")
            break

    if mod_name == '':
        print(f"Could not find module name for module with module number {mod_nr}")
...
def add_modules(prodrisk, data_dir, model_name):
    ...
    vol_head_dict = get_vol_head_dict(data_dir, model_name)
    ...
    for i in range(module_data.size):
        ...
        if(mod_name in vol_head_dict.keys()):
            mod.volHeadCurve.set(pd.Series(name=0.0, index=vol_head_dict[mod_name][0], data=vol_head_dict[mod_name][1]))
