...
def add_hardcoded_module_data(prodrisk):
    # --- add a module to the session ---

    mod = prodrisk.model.module.add_object('South_Norway')
    mod.name.set('South_Norway')
    mod.plantName.set('Production')
    mod.number.set(1001)
    mod.ownerShare.set(1.0)
    mod.regulationType.set(1)
    
    mod.rsvMax.set(12449.9121)
    mod.connectedSeriesId.set(1)
    mod.connected_unreg_series_id.set(2)    
    mod.meanRegInflow.set(1.0)
    mod.meanUnregInflow.set(1.0)
    
    total_max_vol = mod.rsvMax.get()
    max_vol_winter = 12411.4863
    
    max_vol = np.zeros(prodrisk.n_weeks)
    max_vol[0:46] = total_max_vol
    max_vol[46:52] = max_vol_winter
    
    for week in range(prodrisk.n_weeks):
        max_vol[week] = max_vol[week%52]

    mod.maxVol.set(pd.Series(name=0.0,
                         index = [prodrisk.start_time + pd.Timedelta(weeks=i) for i in range(prodrisk.n_weeks)],
                         data=max_vol))
    
    min_vol = np.zeros(prodrisk.n_weeks)
    min_vol[0:17] = 0.0
    min_vol[17:23] = 170.9562
    min_vol[23] = 223.4161
    min_vol[24:26] = 265.2506
    min_vol[26] = 522.6378
    min_vol[27:35] = 545.9977
    min_vol[35] = 223.4161
    min_vol[36] = 170.9562
    min_vol[37:52] = 0.0
    
    for week in range(prodrisk.n_weeks):
        min_vol[week] = min_vol[week%52]
    ...
    mod.minDischarge.set(pd.Series(name=0.0, data=[q_min], index=[prodrisk.start_time]))
...
def build_prodrisk_model(LTM_input_folder, n_weeks=156, start_time="2030-01-07"):
    ...
    add_hardcoded_module_data(prodrisk)
...
def add_inflow_series(prodrisk, data_dir, model_name):
    ...
    inflow_serie = prodrisk.model.inflowSeries.add_object("I-1")
    inflow_serie.seriesId.set(1)
    inflow_serie.histAverageInflow.set(1.0)

    # Inflow series for unregulated inflow
    inflow_serie = prodrisk.model.inflowSeries.add_object("I-2")
    inflow_serie.seriesId.set(2)
    inflow_serie.histAverageInflow.set(1.0)
