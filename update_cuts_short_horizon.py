...
def build_model(prodrisk, n_weeks):
    ...
    prodrisk.prodrisk_path = "C:/PRODRISK/ltm_core_bin/"

    prodrisk.n_scenarios = 10
    prodrisk.min_iterations = 1  # default 1
    prodrisk.max_iterations = 5  # default 10
    prodrisk.min_iterations_first_run = 1  # default 1
    prodrisk.max_iterations_first_run = 5  # default 10
    ...
    scens = []
    scens.append(y1 + y2 + y3)
    scens.append(y2 + y3 + y1)
    scens.append(y3 + y1 + y2)
    scens.append(y1 + y1 + y2)
    scens.append(y1 + y2 + y2)
    scens.append(y2 + y2 + y3)
    scens.append(y2 + y3 + y3)
    scens.append(y3 + y1 + y1)
    scens.append(y3 + y3 + y1)
    scens.append(y2 + y1 + y3)

    inflow_df = pd.DataFrame(
        index=[prodrisk.start_time + pd.Timedelta(weeks=i) for i in range(prodrisk.n_weeks)],
        data={
            f"scen{i}": scens[i][0:prodrisk.n_weeks] for i in range(len(scens))
        },
    )
    ...
    price_df = pd.DataFrame(
        index=[prodrisk.start_time + pd.Timedelta(weeks=i) for i in range(prodrisk.n_weeks)],
        data={
            f"scen{i}": ps[0:prodrisk.n_weeks] for i, ps in enumerate(priceScenIndexed)
        },
    )
...
def set_all_cuts(prodrisk, prodrisk_second_run):
    time_format = "%Y%m%d%H%M"

    dt_cut_time = prodrisk._start_time
    prodriskCore = prodrisk._pb_api
    prodriskCore_sim_run = prodrisk_second_run._pb_api
    ...
    for week in range(prodrisk._n_weeks):
        ...
        my_area_sim_run = prodrisk_second_run.model.area[area_name]
        ...
        for mod_name in module_names:
            ...
            prodrisk_second_run.model.module[mod_name].cutCoeffs.set(coeffs)
        for series_name in series_names:
            coeffs = prodrisk.model.inflowSeries[series_name].cutCoeffs.get()
            prodrisk_second_run.model.inflowSeries[series_name].cutCoeffs.set(coeffs)

        prodrisk_second_run.max_cuts_created = RHS[0].size
...
if __name__ == "__main__":
    ...
    build_model(prodrisk_second_run, 156)
    ...
    prodrisk_second_run.end_week_new_cuts = 52   # Only update cuts for first 52 weeks (Option#5 in flex menu).
